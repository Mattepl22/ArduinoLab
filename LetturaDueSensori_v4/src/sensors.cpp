...
// Funzioni modificate per funzionare con il taskManager

void dhtTask(void *param) {
    Sistema *s = (Sistema *)param; // Cast del parametro generico al tipo reale

    if (timerTrigger(&s->dhtSensor.timer)) {
        float temperature = s->dhtSensor.dht->readTemperature();
        Serial.println(temperature);

        digitalWrite(s->ledPin.pin1, temperature < SOGLIA_TEMPERATURA);
        digitalWrite(s->ledPin.pin2, temperature >= SOGLIA_TEMPERATURA);
    }
}

void potTask(void *param) {
    Sistema *s = (Sistema *)param;

    if (timerTrigger(&s->potenziometro.timer)) {
        int newVal = mediaMobileUpdate(&s->potenziometro.mediaMobile, analogRead(s->potenziometro.pin));

        if ((newVal != s->potenziometro.oldAnalogRead) && s->potenziometro.mediaMobile.full) {
            Serial.println(newVal);
            s->potenziometro.oldAnalogRead = newVal;
        }   
    }
}